...
import '../../configured_agents/configured_agent_factory.dart';
...
class A2AHostService {
  ...
  /// Replaces the set of agents this host serves.
  ///
  /// Safe to call while running: only the routing table is rebuilt, so the
  /// socket stays bound, paired clients stay authorized, and the port does
  /// not change. Sharing is toggled one agent at a time from each agent's
  /// page, and a restart on every toggle would drop peers mid-conversation.
  Future<void> setSharedAgents(List<SavedAgentConfig> agents) async {
    final factory = _services.getRequiredService<ConfiguredAgentFactory>();
    final built = <String, HostedAgent>{};
    for (final config in agents) {
      ...
      built[path] = HostedAgent._(
        path,
        config,
        a2a.A2AJsonRpcTransportHandler(requestHandler),
      );
    }
    // Swapped in one step, after every agent has been built: a half-built
    // table would answer a request that arrives mid-rebuild with a 404.
    _agentsByPath
      ..clear()
      ..addAll(built);
  }
