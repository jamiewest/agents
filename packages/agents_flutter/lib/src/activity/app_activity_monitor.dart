...
/// through [reportUserActivity] and foreground transitions through
/// [reportForeground] (mapping its lifecycle state to a bool), and the chat
/// provider brackets each model turn with [beginInference]/[endInference].
class AppActivityMonitor {
  ...
  ///
  /// A move out of the foreground also counts as activity so background work
  /// stops when the app is backgrounded. The host maps its lifecycle signal
  /// to a bool (in Flutter, `state == AppLifecycleState.resumed`).
  void reportForeground(bool foreground) {
    if (!foreground) _emit();
